...
Widget _buildEventsBannerSection() {
  return StreamBuilder<QuerySnapshot>(
    stream: FirebaseFirestore.instance.collection('event-database').snapshots(),
    builder: (context, snapshot) {
      ...
      final events = snapshot.data!.docs.where((doc) {
        ...
        return data.containsKey('event-banner') && data['event-banner'].toString().isNotEmpty;
      }).toList();
      ...
      return Column(
        ...
        children: [
          SizedBox(
            height: 160, // Increased to match banner size
            child: ListView.builder(
              scrollDirection: Axis.horizontal,
              itemCount: events.length,
              itemBuilder: (context, index) {
                final eventData = events[index].data() as Map<String, dynamic>;
                final bannerUrl = eventData['event-banner'].toString();
                return Padding(
                  ...
                  child: GestureDetector(
                    onTap: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => BookEventPage(eventData: eventData),
                        ),
                      );
                    },
                    child: ClipRRect(
          ...
          const SizedBox(height: 24), // Gap after the banner
        ],
      );
    },
  );
    ...
    return StreamBuilder<List<QuerySnapshot>>(
      ...
      builder: (context, snapshot) {
        ...
        for (var doc in bookmarksSnapshot.docs) {
          final data = doc.data() as Map<String, dynamic>;
          if (data['genre'] != null)
            genres.add(data['genre'].toString().toLowerCase());
          if (data['author'] != null)
            authors.add(data['author'].toString().toLowerCase());
        ...
        final contentBooks =
            booksSnapshot.docs
                .where((doc) {
                  ...
                  return (genres.contains(genre) || authors.contains(author)) &&
                      !userBookIds.contains(doc.id);
                })
                .map(
                  (doc) => {
                    'id': doc.id,
        ...
        for (var doc in ratingsSnapshot.docs) {
          ...
          otherUsersRatings.putIfAbsent(otherUserId, () => {})[doc.id] =
              doc['rating'];
        }
        ...
        for (var entry in otherUsersRatings.entries) {
          ...
          final similarity =
              normA > 0 && normB > 0
                  ...
                  : 0;
          if (similarity > 0)
            userSimilarities[otherUserId] = similarity.toDouble();
        }

        // Get top 5 similar users
        final similarUsers =
            userSimilarities.entries
                .toList()
        ...
        final collabBooks =
            ratingsSnapshot.docs
                .where(
                  (doc) =>
                      similarUsers.contains(doc.reference.parent.parent!.id),
                )
                .where((doc) => (doc['rating'] ?? 0) >= 4)
                .where((doc) => !userBookIds.contains(doc.id))
                .map((doc) {
                  final book = booksSnapshot.docs.firstWhereOrNull(
                    (b) => b.id == doc.id,
                  );
        ...
        allRecommendations.addAll(
          contentBooks.map(
            (b) => {...b, 'score': ((b['score'] ?? 0) as double) * 0.75},
          ),
        );
        allRecommendations.addAll(
          collabBooks.map((b) => {...b, 'score': b['score'] * 0.25}),
        );
        ...
        for (var rec in allRecommendations) {
          ...
          if (bookMap.containsKey(id)) {
            bookMap[id]!['score'] =
                (bookMap[id]!['score'] as double) + (rec['score'] as double);
          } else {
            bookMap[id] = rec;
          }
        }
        final recommendedBooks =
            bookMap.values.toList()
              ..sort(
                (a, b) =>
